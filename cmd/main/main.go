...
import (
	...
	searchMicroservice "main/internal/microservices/search/proto"
...
func main() {
	...
	searchConn, err := grpc.Dial(
		os.Getenv("SEARCH_HOST")+":"+
			os.Getenv("SEARCH_PORT"),
		grpc.WithTransportCredentials(insecure.NewCredentials()),
	)

	if err != nil {
		echoServer.Logger.Fatal("search cant connect to grpc")
	}

	searchManager := searchMicroservice.NewSearchClient(searchConn)

	searchHandlers := api.NewSearchHandler(logger, searchManager)
	searchHandlers.Register(echoServer)
