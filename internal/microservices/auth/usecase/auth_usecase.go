...
func (s *Service) SignUp(ctx context.Context, data *proto.SignUpData) (*proto.EmailLink, error) {
	if err := validation.ValidateUser(data); err != nil {
		return &proto.EmailLink{Link: ""}, status.Error(codes.Internal, err.Error())
	}
	...
	if !isUnique {
		return &proto.EmailLink{Link: ""}, status.Error(codes.InvalidArgument, constants.ErrEmailIsNotUnique.Error())
	}
	...
	msg := "Подтвердите Email\r\n" +
		...
		"https://" + os.Getenv("HOST") + "/confirm?hash=" + hash.Hash
	...
	if err != nil {
		err2 := s.storage.DeleteUser(hash)
		if err2 != nil {
			return &proto.EmailLink{Link: ""}, status.Error(codes.Internal, err2.Error())
		}
		return &proto.EmailLink{Link: ""}, status.Error(codes.Internal, constants.ErrTryAgain.Error())
	}
...
func (s *Service) LogIn(ctx context.Context, data *proto.LogInData) (*proto.Cookie, error) {
	...
	if err != nil {
		if errors.Is(err, constants.ErrWrongData) {
			return &proto.Cookie{Cookie: ""}, status.Error(codes.NotFound, err.Error())
		}
		if errors.Is(err, constants.ErrEmailIsNotConfirmed) {
			return &proto.Cookie{Cookie: ""}, status.Error(codes.Unauthenticated, err.Error())
		}
		return &proto.Cookie{Cookie: ""}, status.Error(codes.Internal, err.Error())
	}
